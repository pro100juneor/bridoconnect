...
interface StreamItem extends StreamRow {
  host_name: string;
  host_flag: string;
}
          ...
          <div className="px-4 mt-4 space-y-4">
            {streams.map((s, idx) => {
              ...
              return (
                ...
                >
                  <div
                    className={`${isHero ? "h-52" : "h-36"} bg-primary/5 flex items-center justify-center relative`}
                  >
                    <Radio
                      className={`${isHero ? "w-16 h-16" : "w-12 h-12"} text-primary/20`}
                      strokeWidth={1.75}
                      aria-hidden="true"
                    />
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                      <div
                        className={`${isHero ? "w-16 h-16" : "w-14 h-14"} rounded-full bg-white/90 shadow-lg flex items-center justify-center`}
                      >
                        <Play
                          className={`${isHero ? "w-7 h-7" : "w-6 h-6"} text-accent fill-accent ml-1`}
                          strokeWidth={1.75}
                        />
                      </div>
                    </div>
                  ...
                  <div className="p-3">
                    <p
                      className={`font-semibold text-foreground mb-1 line-clamp-2 ${isHero ? "text-base" : "text-sm"}`}
                    >
                      {s.title}
                    </p>
