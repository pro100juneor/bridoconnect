...
import { ArrowLeft, Star, MessageCircle, Heart, CheckCircle } from "lucide-react";
      ...
      <div className="px-4 py-6 text-center">
        ...
        <div className="mt-5 space-y-2">
          <div className="relative bg-secondary rounded-2xl px-4 py-3 overflow-hidden before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-white/8">
            <span className="text-[10px] uppercase tracking-widest text-muted-foreground block mb-1">Допомогли</span>
      ...
      {deals.length > 0 ? (
        ...
        <div className="px-4 mb-6">
          ...
          <div className="flex flex-col items-center py-8 gap-3">
            <svg viewBox="0 0 48 48" className="w-12 h-12 text-muted-foreground" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        ...
        <div className="px-4">
          ...
          <div className="space-y-3">
            {reviews.slice(0, 5).map((r: any) => (
              <div key={r.id} className="relative p-3 rounded-2xl border border-border overflow-hidden before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-white/8">
                <div className="flex items-center gap-2 mb-1">
                  <div className="flex">
                    {[1,2,3,4,5].map(i => (
                      <Star key={i} className={`w-3.5 h-3.5 ${i <= r.rating ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground"}`} strokeWidth={1.75} />
                    ))}
                  </div>
