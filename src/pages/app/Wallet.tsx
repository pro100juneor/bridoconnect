...
const Wallet = () => {
  ...
  const totalOut = transactions.filter(t => t.type === "deal_payment" || t.type === "withdrawal").reduce((s, t) => s + t.amount, 0);
  const totalIn = transactions.filter(t => t.type === "deposit" || t.type === "refund").reduce((s, t) => s + t.amount, 0);
          ...
          {transactions.map((t) => {
            ...
            return (
              <div key={t.id}
                ...
                className={`flex items-center gap-3 p-3 rounded-xl border border-border ${t.deal_id ? "cursor-pointer hover:bg-secondary/50" : ""}`}>
