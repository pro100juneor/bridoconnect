...
const Dispute = () => {
  ...
  // Підтягую дані про деал для шапки
  useEffect(() => {
    if (!id) return;
    supabase
      .from("deals")
  ...
  return (
    <div className="pb-8">
      ...
      <div className="flex gap-0 mx-4 mt-4 mb-6 rounded-full overflow-hidden bg-secondary h-1.5">
        {[1, 2].map(s => (
          <div key={s} className={`flex-1 transition-colors ${s <= step ? "bg-destructive" : ""}`} />
        ))}
      </div>
          ...
          <div className="space-y-3">
            ...
            {reasons.map(r => (
              <button
                key={r}
                onClick={() => setReason(r)}
                className={`w-full text-left p-4 rounded-xl border transition-colors text-sm ${
                  reason === r
          ...
          <div className="space-y-4">
            <div>
              ...
              <textarea
                ...
                rows={5}
                className="w-full bg-secondary rounded-xl px-4 py-3 text-sm outline-none text-foreground placeholder:text-muted-foreground resize-none"
              />
            </div>
