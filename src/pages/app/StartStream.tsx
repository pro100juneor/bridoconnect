...
const StartStream = () => {
  ...
  const handleStart = async () => {
    ...
    if (ok) {
      ...
      setRoomName("");
      void notify("error");
      alert("Не вдалося підключити ефір. Перевірте доступ до камери/мікрофона і спробуйте знову.");
    }
  };
      ...
      <main className="min-h-screen flex flex-col bg-black relative">
        ...
        <div className="flex items-center justify-between px-4 pt-10 pb-3">
          ...
          <div className="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-full">
            ...
            <span className="text-white text-xs">{Math.max(participants - 1, 0)} глядачів</span>
          </div>
        </div>
          ...
          {status !== "live" && (
            ...
            >
              ...
              <p className="text-white/50 text-sm leading-relaxed">
                {connecting ? "Камера підключається…" : "Очікуємо камеру…"}
              </p>
    ...
    <main className="pb-8">
      ...
      <div className="px-4 space-y-4">
        ...
        <Button
          ...
          onClick={handleStart}
        >
          <Radio className="w-4 h-4" strokeWidth={1.75} /> {connecting ? "Підключаємось…" : "Почати ефір"}
        </Button>
      </div>
    </main>
  );
