...
// DESIGN.md §Cards: stacked shadow (banned: shadow-sm, shadow-md flat).
const CARD_SHADOW = "shadow-[0_1px_2px_rgb(0_0_0/0.05),0_8px_24px_rgb(0_0_0/0.04)]";
// DESIGN.md §Cards: inset-highlight required on every card.
const CARD_INSET = "relative before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-white/8 before:rounded-t-2xl";

export default function HomePage() {
  const [slide, setSlide] = useState(0);
  const [direction, setDirection] = useState(0); // -1 prev, +1 next, used by framer
  const [ri, setRi] = useState(0);
  const reduced = useReducedMotion();

  const go = useCallback((n: number, dir: number) => {
    setDirection(dir);
    setSlide(n);
  }, []);
  const next = useCallback(() => go((slide + 1) % 5, 1), [slide, go]);
  const prev = useCallback(() => go((slide - 1 + 5) % 5, -1), [slide, go]);
  ...
  // Spring-driven horizontal slide; reduced-motion → instant cross-fade.
  const slideVariants = reduced
    ? { enter: { opacity: 0 }, center: { opacity: 1 }, exit: { opacity: 0 } }
    : {
        enter: (d: number) => ({ x: d > 0 ? "100%" : "-100%", opacity: 0.8 }),
  ...
  // DESIGN.md §Animation — hero title word-stagger.
    ...
    <div className="relative overflow-hidden" style={{ height: "calc(100vh - 64px)" }}>
      <AnimatePresence initial={false} custom={direction} mode="popLayout">
        ...
        >
              ...
              <div className="relative z-10 max-w-5xl mx-auto px-8 w-full grid lg:grid-cols-2 gap-12 items-center">
                <div>
                  ...
                  <h1 className="font-serif text-5xl text-white leading-tight mb-5">
                    {heroWords.map((w, i) => (
                      <motion.span
                        ...
                        transition={{ delay: 0.05 + i * 0.07, ease: [0.34, 1.56, 0.64, 1], duration: 0.5 }}
                      >
                        {w}
                      </motion.span>
                    ))}
                    ...
                    <motion.em
                      ...
                      initial={reduced ? false : { opacity: 0, y: 12 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: 0.05 + heroWords.length * 0.07, ease: [0.34, 1.56, 0.64, 1], duration: 0.5 }}
                    >
                      людині
                    </motion.em>
                  </h1>
                  ...
                  <div className="flex flex-wrap gap-3 mb-10">
                    <Link to="/register" className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-white transition-transform duration-150 hover:-translate-y-px" style={{ background: "#e94560" }}>Почати допомагати →</Link>
                    <Link to="/register?role=executor" className="flex items-center gap-2 px-6 py-3 rounded-xl font-semibold text-white/80 transition-transform duration-150 hover:-translate-y-px" style={{ border: "1px solid rgba(255,255,255,0.2)", background: "rgba(255,255,255,0.05)" }}>Мені потрібна допомога</Link>
                  </div>
                ...
                <div className={`hidden lg:block rounded-2xl overflow-hidden ${CARD_INSET}`} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)" }}>
                  ...
                  <div className="p-5">
                    ...
                    <Link to="/register" className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold text-white transition-transform duration-150 hover:-translate-y-px" style={{ background: "#e94560" }}><Plus className="w-3.5 h-3.5" />Допомогти</Link>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* СЛАЙД 2: ЯК ЦЕ ПРАЦЮЄ — asymmetric 2fr/1fr/1.5fr */}
          {slide === 1 && (
            <div className="h-full bg-background flex items-center">
              <div className="max-w-5xl mx-auto px-8 w-full">
                <div className="text-left mb-10 border-l-2 border-accent pl-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr_1.5fr] gap-6">
                  {[
                    { n: "01", icon: Users, t: "Знайди людину", d: "Переглянь профілі верифікованих отримувачів. Читай реальні історії та рейтинг. Обирай сам, без посередників.", tag: "Лише верифіковані", extended: "Кожен профіль перевірений — документи, фото, історія." },
                    ...
                    { n: "03", icon: Eye, t: "Отримай підтвердження", d: "Фото і відео після виконання. Escrow захищає тебе.", tag: "Прозоро і публічно" },
                  ].map((s, idx) => (
                    <div
                      key={s.n}
                    ...
                    >
                      <div className="flex items-center gap-3 mb-4">
                        <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "rgba(233,69,96,0.1)" }}>
                          <s.icon className="w-5 h-5 text-accent" strokeWidth={1.75} />
                        </div>
                        <span className={`font-serif text-accent/20 font-bold ${idx === 0 ? "text-5xl" : "text-3xl"}`}>{s.n}</span>
                      </div>
                      <h3 className={`font-semibold text-foreground mb-2 ${idx === 0 ? "text-xl" : "text-lg"}`}>{s.t}</h3>
                      <p className="text-sm text-muted-foreground leading-relaxed mb-4">{s.d}</p>
                      {s.extended && <p className="text-xs text-muted-foreground/70 leading-relaxed mb-4 italic">{s.extended}</p>}
                ...
                <div className="text-left mt-8"><Link to="/how-it-works" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-border text-sm font-medium hover:bg-secondary transition-colors hover:-translate-y-px duration-150">Детальніше про процес →</Link></div>
              </div>
            </div>
          )}
          ...
          {/* СЛАЙД 3: ОТРИМУВАЧІ */}
          {slide === 2 && (
            <div className="h-full bg-secondary flex items-center">
              <div className="max-w-5xl mx-auto px-8 w-full">
                <div className="text-left mb-8 border-l-2 border-accent pl-4">
                ...
                <div className="grid lg:grid-cols-2 gap-6 items-start">
                  <div className={`bg-card border border-border rounded-2xl overflow-hidden ${CARD_INSET} ${CARD_SHADOW}`}>
                    ...
                    <div className="p-5">
                      ...
                      <Link to="/register" className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold text-white transition-transform duration-150 hover:-translate-y-px" style={{ background: "#e94560" }}><Plus className="w-3.5 h-3.5" />Допомогти {r.name.split(" ")[0]}</Link>
                    </div>
                  </div>
          ...
          {/* СЛАЙД 4: ПРОЗОРІСТЬ — asymmetric 1.5fr/1fr/1fr/1fr */}
          {slide === 3 && (
            <div className="h-full bg-background flex items-center">
              <div className="max-w-5xl mx-auto px-8 w-full">
                <div className="text-left mb-10 border-l-2 border-accent pl-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.5fr_1fr_1fr_1fr] gap-5 mb-8">
                  {[
                    { icon: Shield, n: "100%", l: "Верифіковані", d: "Кожен отримувач проходить перевірку документів. Жодного анонімного профілю.", anchor: true },
                    ...
                    >
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-4" style={{ background: "rgba(233,69,96,0.1)" }}>
                        <p.icon className="w-5 h-5 text-accent" strokeWidth={1.75} />
                      </div>
                      <div className={`font-bold text-foreground ${p.anchor ? "text-3xl" : "text-2xl"}`}>{p.n}</div>
                ...
                <div className={`bg-secondary rounded-2xl p-6 ${CARD_INSET}`}>
                ...
                <div className="text-left mt-6"><Link to="/transparency" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-border text-sm font-medium hover:bg-secondary transition-colors hover:-translate-y-px duration-150">Детальніше →</Link></div>
              </div>
            </div>
          )}
          ...
          {/* СЛАЙД 5: CTA */}
          {slide === 4 && (
            <div className="h-full flex items-center" style={{ background: "#16213e" }}>
              <div className="max-w-3xl mx-auto px-8 w-full text-center">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center mb-10">
                  <Link to="/register" className="flex items-center justify-center gap-2 px-8 py-4 rounded-xl font-bold text-white text-lg transition-transform duration-150 hover:-translate-y-px" style={{ background: "#e94560" }}><Plus className="w-5 h-5" />Зареєструватись</Link>
                  <Link to="/register?role=executor" className="flex items-center justify-center gap-2 px-8 py-4 rounded-xl font-semibold text-white text-lg transition-transform duration-150 hover:-translate-y-px" style={{ border: "1px solid rgba(255,255,255,0.2)", background: "rgba(255,255,255,0.08)" }}>Мені потрібна допомога</Link>
                </div>
      ...
      {/* Навігація */}
      <button onClick={prev} aria-label="Попередній слайд" className="absolute left-4 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full flex items-center justify-center bg-background/80 border border-border hover:bg-background transition-colors"><ChevronLeft className="w-4 h-4" /></button>
      <button onClick={next} aria-label="Наступний слайд" className="absolute right-4 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full flex items-center justify-center bg-background/80 border border-border hover:bg-background transition-colors"><ChevronRight className="w-4 h-4" /></button>
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 z-20">
        {slides.map((_, i) => (
          <button
            ...
            className={`rounded-full transition-all duration-300 ${i === slide ? "w-6 h-2 bg-accent" : "w-2 h-2 bg-foreground/20 hover:bg-foreground/40"}`}
          />
        ))}
      </div>
