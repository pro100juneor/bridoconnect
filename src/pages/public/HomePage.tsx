        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 sm:px-8 w-full grid lg:grid-cols-2 gap-8 lg:gap-12 items-center py-12">
          <div>
            ...
            <h1 className="font-serif text-4xl sm:text-5xl text-white leading-tight mb-5">
              {heroWords.map((w, i) => (
                <motion.span key={i} className="inline-block mr-3" {...wordVariant(i)}>{w}</motion.span>
              ))}
              <br />
              <motion.em className="not-italic" style={{ color: "#e94560" }} {...wordVariant(heroWords.length)}>людині</motion.em>
            </h1>
            ...
            <div className="flex flex-wrap gap-3 mb-10">
              <Link to="/register" className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-white transition-transform duration-150 hover:-translate-y-px min-h-[44px]" style={{ background: "#e94560" }}>Почати допомагати →</Link>
              <Link to="/register?role=executor" className="flex items-center gap-2 px-6 py-3 rounded-xl font-semibold text-white/80 transition-transform duration-150 hover:-translate-y-px min-h-[44px]" style={{ border: "1px solid rgba(255,255,255,0.2)", background: "rgba(255,255,255,0.05)" }}>Мені потрібна допомога</Link>
            </div>
      ...
      <section id="how" className="py-20 bg-background">
        <div className="max-w-5xl mx-auto px-6 sm:px-8">
          <InView className="text-left mb-12 border-l-2 border-accent pl-4">
            ...
            <h2 className="font-serif text-4xl text-foreground tracking-tight">Три кроки до <em className="not-italic text-accent">реальної допомоги</em></h2>
          </InView>
          <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr_1.5fr] gap-6">
            {[
              ...
              <InView key={s.n}>
                <div className={`bg-card border border-border rounded-2xl p-6 transition-all duration-200 hover:-translate-y-px hover:${CARD_SHADOW} ${CARD_INSET}`}>
                  <div className="flex items-center gap-3 mb-4">
          ...
          <InView className="text-left mt-8">
            <Link to="/how-it-works" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-border text-sm font-medium hover:bg-secondary transition-colors min-h-[44px]">Детальніше про процес →</Link>
          </InView>
        </div>
      </section>
      ...
      <section id="recipients" className="py-20 bg-secondary">
        <div className="max-w-5xl mx-auto px-6 sm:px-8">
          <InView className="text-left mb-8 border-l-2 border-accent pl-4">
            ...
            <h2 className="font-serif text-4xl text-foreground tracking-tight">Тобі можуть <em className="not-italic text-accent">допомогти</em></h2>
          </InView>
          <div className="grid lg:grid-cols-2 gap-6 items-start">
            <InView className={`bg-card border border-border rounded-2xl overflow-hidden ${CARD_INSET} ${CARD_SHADOW}`}>
              ...
              <div className="p-5">
                ...
                <Link to="/register" className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold text-white transition-transform duration-150 hover:-translate-y-px min-h-[44px]" style={{ background: "#e94560" }}><Plus className="w-3.5 h-3.5" />Допомогти {r.name.split(" ")[0]}</Link>
              </div>
            </InView>
            <div className="space-y-2">
              {recipients.map((rec, i) => (
                <button key={rec.name} onClick={() => setRi(i)} className={`w-full flex items-center gap-3 p-3 rounded-2xl border text-left transition-all min-h-[44px] hover:-translate-y-px duration-150 ${i === ri ? "border-accent bg-accent/5" : "border-border bg-card hover:border-accent/40"}`}>
              ...
              <Link to="/register?role=executor" className="w-full flex items-center justify-center py-2.5 rounded-2xl border border-border text-sm font-medium hover:bg-secondary transition-colors mt-2 min-h-[44px]">Зареєструватись і опублікувати профіль →</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="transparency" className="py-20 bg-background">
        <div className="max-w-5xl mx-auto px-6 sm:px-8">
          <InView className="text-left mb-10 border-l-2 border-accent pl-4">
            ...
            <h2 className="font-serif text-4xl text-foreground tracking-tight">Кожен цент <em className="not-italic text-accent">на виду</em></h2>
          </InView>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.5fr_1fr_1fr_1fr] gap-5 mb-8">
            {[
              ...
              <InView key={p.n}>
                <div className={`bg-card border border-border rounded-2xl p-5 transition-all duration-200 hover:-translate-y-px ${CARD_SHADOW} ${CARD_INSET} ${p.anchor ? "lg:p-6" : ""}`}>
          ...
          <InView className="text-left mt-6">
            <Link to="/transparency" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-border text-sm font-medium hover:bg-secondary transition-colors min-h-[44px]">Детальніше →</Link>
          </InView>
        </div>
      </section>
      ...
      <section id="cta" className="py-20" style={{ background: "#16213e" }}>
        <div className="max-w-3xl mx-auto px-6 sm:px-8 text-center">
          <InView>
            ...
            <h2 className="font-serif text-4xl sm:text-5xl text-white mb-6 leading-tight tracking-tight">Один крок —<br />і чиєсь життя <em className="not-italic" style={{ color: "#e94560" }}>зміниться</em></h2>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center mb-10">
              <Link to="/register" className="flex items-center justify-center gap-2 px-8 py-4 rounded-xl font-bold text-white text-lg transition-transform duration-150 hover:-translate-y-px min-h-[44px]" style={{ background: "#e94560" }}><Plus className="w-5 h-5" />Зареєструватись</Link>
              <Link to="/register?role=executor" className="flex items-center justify-center gap-2 px-8 py-4 rounded-xl font-semibold text-white text-lg transition-transform duration-150 hover:-translate-y-px min-h-[44px]" style={{ border: "1px solid rgba(255,255,255,0.2)", background: "rgba(255,255,255,0.08)" }}>Мені потрібна допомога</Link>
            </div>
