...
const VerificationPage = () => {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const msg =
        e instanceof Error ? e.message : t("verification.error.unavailable", "Верифікація недоступна");
      // "Stripe Identity" — название сервиса, не переводим
    ...
    return (
      <main className="min-h-screen flex flex-col items-center justify-center px-6 text-center relative">
        ...
        <h1 className="font-serif text-4xl tracking-tight text-foreground mb-2 animate-fade-in">
          {t("verification.verified.title", "Ви верифіковані")}
        </h1>
        <p className="text-muted-foreground text-sm mb-6 leading-relaxed">
          {t("verification.verified.desc", "Значок довіри вже на вашому профілі. Дякуємо!")}
        </p>
        <Button
        ...
        >
          {t("verification.verified.cta", "До профілю")}
        </Button>
      </main>
    );
  }
      ...
      <section className="px-6 py-12 max-w-lg mx-auto">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h1 className="font-serif text-4xl tracking-tight text-foreground animate-fade-in">
            {t("verification.title", "Верифікація акаунту")}
          </h1>
        </div>
        <p className="text-muted-foreground text-sm mb-8 leading-relaxed">
          {t(
            "verification.intro",
            "Верифіковані акаунти отримують значок довіри і мають пріоритет у стрічці. Перевірка триває 2–5 хвилин."
          )}
        </p>
        ...
        <div className="relative p-4 bg-success/10 rounded-2xl mb-6 flex items-start gap-3 overflow-hidden before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-white/8">
          ...
          <div>
            <p className="text-sm font-semibold text-foreground">
              {t("verification.secure.title", "Ваші дані захищені")}
            </p>
            <p className="text-xs text-muted-foreground leading-relaxed">
              {t(
                "verification.secure.body",
                "Перевірку виконує Stripe Identity (GDPR, PCI DSS). BridoConnect отримує лише результат «підтверджено / не підтверджено» — жодних копій документів."
              )}
            </p>
          </div>
        </div>
        ...
        {status === "pending" && (
          <div className="relative p-4 border border-warning/40 bg-warning/5 rounded-2xl mb-6 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-foreground">
                {t("verification.pending.title", "Перевірка триває")}
              </p>
              <p className="text-xs text-muted-foreground leading-relaxed">
                {t("verification.pending.body", "Завершіть кроки на сторінці Stripe, що відкрилась.")}
                {polling ? ` ${t("verification.pending.polling", "Статус оновиться тут автоматично.")}` : ""}
              </p>
            </div>
          </div>
        )}
        ...
        >
          ...
          {loading
            ? t("verification.cta.loading", "Готуємо сесію…")
            : status === "pending"
              ? t("verification.cta.continue", "Продовжити верифікацію")
              : t("verification.cta.start", "Пройти верифікацію")}
        </Button>
